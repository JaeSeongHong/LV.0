...
//2. 받은 채팅을 다른사용자(클라이언트)들한테도 보내주는 것
public class SocketServer extends Thread{
	
	private ArrayList<Socket> list = new ArrayList<Socket>(); // 현재까지 연결된 사용자(클라이언트)-Socket 기준 저장용
	private Socket socket = null; //현재 채팅을 보내는/받는 사용자 임시 저장공간
	
	//클라이언트의 소켓정보를 넘기면서 해당 클라잉너트와 데이터 송수신할 수 있는 소켓 생성
	...
	public void run() {
		//클라이언트가 접속하면 해당 클라이언트PC IP주소를 출력!
