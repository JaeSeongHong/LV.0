...
public class IntStack {
	...
	public IntStack(int maxlen) {//스택을 생성할 때 크기를 지정할 수 있도록!
		...
		capacity = maxlen;  //지정된 크기로 스택용량 설정
	...
	public int push(int x) {//원하는 값(x)를 스택에 삽입, 0번 인덱스부터 차근차근 채우기!
		stk[ptr++] = x; // 포인터가 가리키는 위치에 값을 삽입한 후 포인터를 1증가
						// ptr++ : 데이터 개수 카운팅 , 그다음 데이터는 그 다음칸에 들어갈 수 있도록!
		return x; //삽입된 데이터 반환
	}
	...
	//제일 위에 있는 데이터 확인 
	public int peek() {
		return stk[-1]; //ptr(현재까지 데이터 수) 바꾸면 안됨!
	}
	...
	//특정 값이 몇번 인덱스에 있는지 찾기 : TOP(위에서부터!!!)
	public int indexOf(int x) {//x : 찾고싶은 값
		for(int i=ptr-1;i>=0;i--) {//제일 위에 있는 데이터~ 0번까지 1칸씩 내려오면서 탐색
			if(stk[i] == x) { //찾고자 하는 데이터와 같은 값을 발견했을때
				return i;	  //그 데이터가 있는 인덱스번호 반환
			}
			
		}
		return -1; //찾고자 하는 스텍에 없는 경우에는 -1반환
	}
	...
	public boolean isFull() {
		return ptr == capacity; //ptr이 capacity같으면 (가득 차있으면) -> true 반환
